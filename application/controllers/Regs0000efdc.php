    ...
    class Regs0000efdc extends MY_Controller {
      ...
      public function index(){
        ...
        if ($this->session->flashdata('response')){
          ...
          if($this->data['response']['status'] == 'FALSE'){
            $this->data['response']['data'] = $this->sendGet('api/regs0000efdc/get', $this->data['login']->data->token, true)->data;
          }
        } else {
          $this->data['response'] = $this->sendGet('api/regs0000efdc/get', $this->data['login']->data->token, true);
        }
    
        $this->scripts('assets/javascript/api/regs0000efdc/regs0000efdc.js');
        $this->load->view('dashboard/template/header', $this->data);
        $this->load->view('api/regs0000efdc/GridRegs0000efdc', $this->data);
      ...
      public function get($IdParent, $Id = ''){
        if(empty($Id))
          echo json_encode(['data' => $this->sendGet('api/regs0000efdc/getByParent/'.$IdParent, $this->data['login']->data->token)->data]);
        else 
          echo json_encode(['data' => $this->sendGet('api/regs0000efdc/getByParent/'.$IdParent.'/'.$Id, $this->data['login']->data->token)->data]);
      }
      ...
      public function add($parentView, $IdParent){
        if($_POST){
          ...
          if($response['status'] == 'FALSE'){
            redirect('regs0000efdc/create/'.$parentView.'/'.$IdParent);
          } else {
            $salvarEVoltar ? redirect('ref311/'.$parentView.'/'.$IdParent) : redirect('regs0000efdc/edit/'.$parentView.'/'.$IdParent.'/'.$response['data'][0]['0000_Id']);
          }
        }
      }
      ...
      public function update($parentView, $IdParent, $Id){
        if($_POST){
          ...
          if($response['status'] == 'FALSE'){
            redirect('regs0000efdc/edit/'.$parentView.'/'.$IdParent.'/'.$Id);
          } else {
            $salvarEVoltar ? redirect('ref311/'.$parentView.'/'.$IdParent) : redirect('regs0000efdc/edit/'.$parentView.'/'.$IdParent.'/'.$Id);
          }
        }
      }
      ...
      public function delete($parentView, $IdParent){
        if($_POST){
          ...
          $response = $this->sendGet('api/regs0000efdc/getByParent/'.$IdParent.'/'.$Id, $this->data['login']->data->token, true);    
          ...
          if(empty($response['data'])){
            ...
            $response = $this->sendDelete('api/regs0000efdc/delete/'.$Id, $this->data['login']->data->token, true);
            $response['comeFromChild'] = 'TRUE';
            $this->session->set_flashdata('response', $response); 
            redirect('ref311/'.$parentView.'/'.$IdParent);
          }
        }
      }
      ...
      public function view($parentView, $IdParent, $Id){
        ...
        $this->data['IdParent'] = $IdParent;
        $this->data['parentView'] = $parentView;
        $this->data['response'] = $this->sendGet('api/regs0000efdc/getByParent/'.$IdParent.'/'.$Id, $this->data['login']->data->token, true);


        $this->load->view('dashboard/template/header', $this->data);
        $this->load->view('api/regs0000efdc/ViewRegs0000efdc', $this->data);
        $this->load->view('dashboard/template/footer', $this->data);
      }
    }
