            ...
            <div class='page-section'>
              ...
              <div class='section-block'>            
                ...
                <div id='base-style' class='card'>
                  ...
                  <div class='card-body'>
                    <!-- .form -->
                    <?= form_open(base_url('Filha/update/'.$parentView.'/'.$IdParent.'/'.$response['data'][0]['pfa_Id'])); ?>
                      <!-- .fieldset -->
                      <fieldset>
                        <legend>Alteração do registro</legend> <!-- .form-group -->
							<div class='form-group'>
								<label for='pfa_PaiId'>Tabela Pai</label>
							<select name='pfa_PaiId' id='pfa_PaiId' class='custom-select' placeholder='Tabela Pai' required>
								<?= getOptionToSelect('pai','pp_Id', '', $response['data'][0]['pfa_PaiId'], $login->data->token) ?>
							</select>
							<?php if(isset($response)): ?>
							...
							<input type='text'  name='pfa_Nome' id='pfa_Nome' class='form-control' placeholder='Nome' value='<?= $response['data'][0]['pfa_Nome'] ?>' required>
							<?php if(isset($response)): ?>
							...
							<input type='number' step='0.01' name='pfa_Valor' id='pfa_Valor' class='form-control' placeholder='Valor' value='<?= $response['data'][0]['pfa_Valor'] ?>' required>
							<?php if(isset($response)): ?>
							...
							<input type='number' step='0.01' name='pfa_Total' id='pfa_Total' class='form-control' placeholder='Total' value='<?= $response['data'][0]['pfa_Total'] ?>' required>
							<?php if(isset($response)): ?>
							...
							<input type='datetime-local'  name='pfa_DtNascimento' id='pfa_DtNascimento' class='form-control' placeholder='Data Nascimento' value='<?= (substr($response['data'][0]['pfa_DtNascimento'],10,1) !== 'T' ? date_format(date_create_from_format('Y-m-d H:i:s', $response['data'][0]['pfa_DtNascimento']),'Y-m-d\TH:i') : $response['data'][0]['pfa_DtNascimento']) ?>' required>
							<?php if(isset($response)): ?>
							...
							<input type='date'  name='pfa_DtCadastro' id='pfa_DtCadastro' class='form-control' placeholder='Data Cadastro' value='<?= $response['data'][0]['pfa_DtCadastro'] ?>' required>
							<?php if(isset($response)): ?>
							...
							<select name='pfa_Sexo' id='pfa_Sexo' class='custom-select' placeholder='Sexo' required>
								...
								<option value='Masculino' <?= $response['data'][0]['pfa_Sexo'] == 'Masculino' ? 'selected' : '' ?>> Masculino </option>
								<option value='Feminino' <?= $response['data'][0]['pfa_Sexo'] == 'Feminino' ? 'selected' : '' ?>> Feminino </option>
							</select>
						</div>
