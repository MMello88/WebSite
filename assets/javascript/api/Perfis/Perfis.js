...
function () {
  function DTPerfis() {
    _classCallCheck(this, DTPerfis);
    this.init();
  }

  _createClass(DTPerfis, [{
    key: "init",
    value: function init() {
    ...
    value: function table() {
      return $('#myTablePerfis').DataTable({
        dom: "<'text-muted'Bi>\n        <'table-responsive'tr>\n        <'mt-4'p>",
        buttons: ['copyHtml5', {
          extend: 'print', 
          autoPrint: false
        ...
        columnDefs: [{
          ...
          render: function render(data, type, row, meta) {
            ...
            <div class='custom-control custom-control-nolabel custom-checkbox'>
              <input type='checkbox' class='custom-control-input' name='selectedRow[]' id='p${row['ps_Id']}' value='${row['ps_Id']}'>
              <label class='custom-control-label' for='p${row['ps_Id']}'></label>
            </div>`;
          }
        },{
          targets: 1,
    ...
    value: function clearSelected() {
      ...
      $('#myTablePerfis').on('page.dt', function () {
        self.clearSelectedRows();
      });
