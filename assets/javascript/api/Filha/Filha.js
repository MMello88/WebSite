...
function () {
  function DTFilha() {
    _classCallCheck(this, DTFilha);
    this.init();
  }

  _createClass(DTFilha, [{
    key: "init",
    value: function init() {
        ...
        columnDefs: [{
          ...
          render: function render(data, type, row, meta) {
            ...
            <div class='custom-control custom-control-nolabel custom-checkbox'>
              <input type='checkbox' class='custom-control-input' name='selectedRow[]' id='p${row['pfa_Id']}' value='${row['pfa_Id']}'>
              <label class='custom-control-label' for='p${row['pfa_Id']}'></label>
            </div>`;
          }
        },{
          targets: 1,
          render: function render(data, type, row, meta) {
            return `
            <a class='btn btn-sm btn-icon btn-secondary' href='${url_view_filha}/${data}'>
              <i class='fa fa-eye'></i>
            </a>
            <a class='btn btn-sm btn-icon btn-secondary' href='${url_upd_filha}/${data}'>
              <i class='fa fa-pencil-alt'></i>
            </a>
            <a class='btn btn-sm btn-icon btn-secondary' id='btnDeleteClick' data-toggle='modal' data-target='#modalDeleteRegistrofilha' data-pfa_id='${data}' href='#${data}'>
    ...
    value: function setDeleteClick(){
      var self = this;
      $('#modalDeleteRegistrofilha').on('show.bs.modal', function (event) {
        document.getElementById('DeleteBypfa_id').value = $(event.relatedTarget).data('pfa_id');
      })
    }
  },{
    key: "searchRecords",
    ...
    value: function clearSelected() {
      ...
      $('#myTableFilha').on('page.dt', function () {
        self.clearSelectedRows();
      });
