    ...
    class Regs0110efdc extends MY_Controller {
      ...
      public function get(){
        echo json_encode(['data' => $this->sendGet('api/regs0110efdc/get', $this->data['login']->data->token)->data]);
      }
      ...
      public function add(){
        if($_POST){
          ...
          if($response['status'] == 'FALSE'){
            redirect('regs0110efdc/create');
          } else {
            $salvarEVoltar ? redirect('regs0110efdc') : redirect('regs0110efdc/edit/'.$response['data'][0]['0110_Id']);
          }
        }
      }
      ...
      public function update($Id){
        if($_POST){
          ...
          if($response['status'] == 'FALSE'){
            redirect('regs0110efdc/edit/'.$Id);
          } else {
            $salvarEVoltar ? redirect('regs0110efdc') : redirect('regs0110efdc/edit/'.$Id);
          }
        }
      }
      ...
      public function delete(){
        if($_POST){
          ...
          $response = $this->sendGet('api/regs0110efdc/get/'.$Id, $this->data['login']->data->token, true);    
