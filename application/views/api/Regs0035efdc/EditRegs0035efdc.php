                    ...
                    <?= form_open(base_url('Regs0035efdc/update/'.$response['data'][0]['0035_Id'])); ?>
                      ...
                      <fieldset>
                        <legend>Alteração do registro</legend> <!-- .form-group -->
							...
							<input type='number' step='1' name='0035_CodSCP' id='0035_CodSCP' class='form-control' placeholder='Identificação da SCP' value='<?= $response['data'][0]['0035_CodSCP'] ?>' required>
							<?php if(isset($response)): ?>
							...
							<select name='0035_PessoaJuridicaId' id='0035_PessoaJuridicaId' class='custom-select' placeholder='Pessoa Juridica' >
								<?= getOptionToSelect('pessoasjuridica','pj_Id', '', $response['data'][0]['0035_PessoaJuridicaId'], $login->data->token) ?>
							</select>
							<?php if(isset($response)): ?>
