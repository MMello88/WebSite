...
function () {
  function DTColunas() {
    _classCallCheck(this, DTColunas);
    this.init();
  }

  _createClass(DTColunas, [{
    key: "init",
    value: function init() {
    ...
    value: function table() {
      return $('#myTableColunas').DataTable({
        dom: "<'text-muted'Bi>\n        <'table-responsive'tr>\n        <'mt-4'p>",
        buttons: ['copyHtml5', {
          extend: 'print', 
          autoPrint: false
        ...
        columnDefs: [{
          ...
          render: function render(data, type, row, meta) {
            ...
            <div class='custom-control custom-control-nolabel custom-checkbox'>
              <input type='checkbox' class='custom-control-input' name='selectedRow[]' id='p${row['cln_Id']}' value='${row['cln_Id']}'>
              <label class='custom-control-label' for='p${row['cln_Id']}'></label>
            </div>`;
          }
        },{
          targets: 1,
    ...
    value: function clearSelected() {
      ...
      $('#myTableColunas').on('page.dt', function () {
        self.clearSelectedRows();
      });
