    ...
    class Regsa120efdc extends MY_Controller {
      ...
      public function get(){
        echo json_encode(['data' => $this->sendGet('api/regsa120efdc/get', $this->data['login']->data->token)->data]);
      }
      ...
      public function add(){
        if($_POST){
          ...
          if($response['status'] == 'FALSE'){
            redirect('regsa120efdc/create');
          } else {
            $salvarEVoltar ? redirect('regsa120efdc') : redirect('regsa120efdc/edit/'.$response['data'][0]['a120_Id']);
          }
        }
      }
      ...
      public function update($Id){
        if($_POST){
          ...
          if($response['status'] == 'FALSE'){
            redirect('regsa120efdc/edit/'.$Id);
          } else {
            $salvarEVoltar ? redirect('regsa120efdc') : redirect('regsa120efdc/edit/'.$Id);
          }
        }
      }
