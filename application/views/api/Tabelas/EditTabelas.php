            ...
            <div class='page-section'>
              ...
              <div class='section-block'>            
                ...
                <div id='base-style' class='card'>
                  ...
                  <div class='card-body'>
                    <!-- .form -->
                    <?= form_open(base_url('Tabelas/update/'.$response['data'][0]['TABLE_NAME'])); ?>
                      <!-- .fieldset -->
                      <fieldset>
                        <legend>Alteração do registro</legend> <!-- .form-group -->
							<div class='form-group'>
								<label for='AUTO_INCREMENT'>Auto Incremento</label>
							<input type='' name='AUTO_INCREMENT' id='AUTO_INCREMENT' class='form-control' placeholder='Auto Incremento' value='<?= $response['data'][0]['AUTO_INCREMENT'] ?>' >
							<?php if(isset($response)): ?>
							...
							<input type='datetime-local' name='CREATE_TIME' id='CREATE_TIME' class='form-control' placeholder='Data de Criação' value='<?= $response['data'][0]['CREATE_TIME'] ?>' >
							<?php if(isset($response)): ?>
							...
							<input type='datetime-local' name='UPDATE_TIME' id='UPDATE_TIME' class='form-control' placeholder='Data de Alteração' value='<?= $response['data'][0]['UPDATE_TIME'] ?>' >
							<?php if(isset($response)): ?>
							...
							<input type='text' name='TABLE_COMMENT' id='TABLE_COMMENT' class='form-control' placeholder='Descrição da Tabela' value='<?= $response['data'][0]['TABLE_COMMENT'] ?>' required>
							<?php if(isset($response)): ?>
							...
							<select name='TABLE_DELETED' id='TABLE_DELETED' class='custom-select' placeholder='Ativa' >
								...
								<option value='TRUE' <?= $response['data'][0]['TABLE_DELETED'] == 'TRUE' ? 'selected' : '' ?>> TRUE </option>
								<option value='FALSE' <?= $response['data'][0]['TABLE_DELETED'] == 'FALSE' ? 'selected' : '' ?>> FALSE </option>
							</select>
						</div>
							<div class='form-group'>
								<label for='CLASS_NAME'>Nome da Classe</label>
							<input type='text' name='CLASS_NAME' id='CLASS_NAME' class='form-control' placeholder='Nome da Classe' value='<?= $response['data'][0]['CLASS_NAME'] ?>' >
							<?php if(isset($response)): ?>
