...
function () {
  function DTPaises() {
    _classCallCheck(this, DTPaises);
    this.init();
  }

  _createClass(DTPaises, [{
    key: "init",
    value: function init() {
    ...
    value: function table() {
      return $('#myTablePaises').DataTable({
        dom: "<'text-muted'Bi>\n        <'table-responsive'tr>\n        <'mt-4'p>",
        buttons: ['copyHtml5', {
          extend: 'print', 
          autoPrint: false
        ...
        columnDefs: [{
          ...
          render: function render(data, type, row, meta) {
            ...
            <div class='custom-control custom-control-nolabel custom-checkbox'>
              <input type='checkbox' class='custom-control-input' name='selectedRow[]' id='p${row['pss_Id']}' value='${row['pss_Id']}'>
              <label class='custom-control-label' for='p${row['pss_Id']}'></label>
            </div>`;
          }
        },{
          targets: 1,
    ...
    value: function clearSelected() {
      ...
      $('#myTablePaises').on('page.dt', function () {
        self.clearSelectedRows();
      });
