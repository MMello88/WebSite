    ...
    class Usersgrupo extends MY_Controller {
      ...
      public function index(){
        ...
        if ($this->session->flashdata('response')){
          ...
          if($this->data['response']['status'] == 'FALSE'){
            $this->data['response']['data'] = $this->sendGet('api/usersgrupo/get', $this->data['login']->data->token, true)->data;
          }
        } else {
          $this->data['response'] = $this->sendGet('api/usersgrupo/get', $this->data['login']->data->token, true);
        }
    
        $this->scripts('assets/javascript/api/usersgrupo/usersgrupo.js');
        $this->load->view('dashboard/template/header', $this->data);
        $this->load->view('api/usersgrupo/GridUsersgrupo', $this->data);
      ...
      public function create($parentView, $IdParent){
        ...
        $this->load->view('dashboard/template/header', $this->data);
        $this->load->view('api/usersgrupo/CreateUsersgrupo', $this->data);
        $this->load->view('dashboard/template/footer', $this->data);
      }
      ...
      public function edit($parentView, $IdParent, $Id){
        ...
        if(empty($this->data['response']['data'])){
          ...
          $this->load->view('dashboard/template/header', $this->data);
          $this->load->view('api/usersgrupo/EditUsersgrupo', $this->data);
          $this->load->view('dashboard/template/footer', $this->data);
        }
      }
      ...
      public function delete($parentView, $IdParent){
        if($_POST){
          ...
          $response = $this->sendGet('api/usersgrupo/getByParent/'.$IdParent.'/'.$Id, $this->data['login']->data->token, true);    
          ...
          if(empty($response['data'])){
            ...
            $response = $this->sendDelete('api/usersgrupo/delete/'.$Id, $this->data['login']->data->token, true);
            $response['comeFromChild'] = 'TRUE';
            $this->session->set_flashdata('response', $response); 
            redirect('grupousers/'.$parentView.'/'.$IdParent);
          }
        }
      }
      ...
      public function view($parentView, $IdParent, $Id){
        ...
        $this->data['IdParent'] = $IdParent;
        $this->data['parentView'] = $parentView;
        $this->data['response'] = $this->sendGet('api/usersgrupo/getByParent/'.$IdParent.'/'.$Id, $this->data['login']->data->token, true);
