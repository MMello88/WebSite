...
function () {
  function DTFilho() {
    _classCallCheck(this, DTFilho);
    this.init();
  }

  _createClass(DTFilho, [{
    key: "init",
    value: function init() {
        ...
        columnDefs: [{
          ...
          render: function render(data, type, row, meta) {
            ...
            <div class='custom-control custom-control-nolabel custom-checkbox'>
              <input type='checkbox' class='custom-control-input' name='selectedRow[]' id='p${row['pfo_Id']}' value='${row['pfo_Id']}'>
              <label class='custom-control-label' for='p${row['pfo_Id']}'></label>
            </div>`;
          }
        },{
          targets: 1,
          render: function render(data, type, row, meta) {
            return `
            <a class='btn btn-sm btn-icon btn-secondary' href='${url_view_filho}/${data}'>
              <i class='fa fa-eye'></i>
            </a>
            <a class='btn btn-sm btn-icon btn-secondary' href='${url_upd_filho}/${data}'>
              <i class='fa fa-pencil-alt'></i>
            </a>
            <a class='btn btn-sm btn-icon btn-secondary' id='btnDeleteClick' data-toggle='modal' data-target='#modalDeleteRegistrofilho' data-pfo_id='${data}' href='#${data}'>
    ...
    value: function setDeleteClick(){
      var self = this;
      $('#modalDeleteRegistrofilho').on('show.bs.modal', function (event) {
        document.getElementById('DeleteBypfo_id').value = $(event.relatedTarget).data('pfo_id');
      })
    }
  },{
    key: "searchRecords",
    ...
    value: function clearSelected() {
      ...
      $('#myTableFilho').on('page.dt', function () {
        self.clearSelectedRows();
      });
