    ...
    class Pessoasjuridica extends MY_Controller {
      ...
      public function get(){
        echo json_encode(['data' => $this->sendGet('api/pessoasjuridica/get', $this->data['login']->data->token)->data]);
      }
      ...
      public function add(){
        if($_POST){
          ...
          if($response['status'] == 'FALSE'){
            redirect('pessoasjuridica/create');
          } else {
            $salvarEVoltar ? redirect('pessoasjuridica') : redirect('pessoasjuridica/edit/'.$response['data'][0]['pj_Id']);
          }
        }
      }
      ...
      public function update($Id){
        if($_POST){
          ...
          if($response['status'] == 'FALSE'){
            redirect('pessoasjuridica/edit/'.$Id);
          } else {
            $salvarEVoltar ? redirect('pessoasjuridica') : redirect('pessoasjuridica/edit/'.$Id);
          }
        }
      }
      ...
      public function delete(){
        if($_POST){
          ...
          $response = $this->sendGet('api/pessoasjuridica/get/'.$Id, $this->data['login']->data->token, true);    
